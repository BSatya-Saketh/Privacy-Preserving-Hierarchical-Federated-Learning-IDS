...
def build_pdf(filename="Project_Interview_Defense_Guide.pdf"):
    ...
    # ================= 1. PROJECT OVERVIEW =================
    story.append(Paragraph("1. Project Overview (Round 1)", h1_style))
    ...
    story.append(Paragraph(
        "A: Centralized network intrusion detection (NIDS) violates privacy by transmitting raw packet logs to a central server. "
        "It also creates high bandwidth costs and latency bottlenecks. Furthermore, standard centralized supervised ML models "
        "suffer from: (1) target column leakages that cheat during training, (2) zero-support data splits that produce "
        "deceptive accuracy metrics, and (3) a high rate of false alarm rates due to rigid threshold metrics.",
        a_style
    ))
    ...
    # ================= 3. DEEP LEARNING =================
    story.append(PageBreak())
    story.append(Paragraph("3. Deep Learning & Sequence Models", h1_style))
    ...
    # ================= 8. EXPLAINABILITY =================
    story.append(Paragraph("8. Explainability", h1_style))

    story.append(Paragraph("Q1: Why SHAP?", q_style))
    story.append(Paragraph(
        "A: SHAP (SHapley Additive exPlanations) is based on cooperative game theory. It provides mathematically consistent "
        "feature attribution values, ensuring that features are ranked reliably. LIME is unstable in comparison.",
        a_style
    ))
    ...
    # ================= 9. OPTIMIZATION =================
    story.append(PageBreak())
    story.append(Paragraph("9. Optimization", h1_style))

    story.append(Paragraph("Q1: Why float16?", q_style))
    story.append(Paragraph(
        "A: Casting model weights to half-precision (`float16`) reduces the parameter payload size by approximately 50%, "
        "halving bandwidth costs over cellular/satellite IoT connection links.",
        a_style
    ))

    story.append(Paragraph("Q2: What performance improvement did it provide?", q_style))
    story.append(Paragraph(
        "A: It halved the size of local weights dictionary files (from 32-bit floats to 16-bit), reducing transmission payloads "
        "and decreasing training/relay network latency across gateways.",
        a_style
    ))

    story.append(Paragraph("Q3: Would INT8 be better?", q_style))
    story.append(Paragraph(
    ...
    story.append(Paragraph("Q4: What other optimizations would you make?", q_style))
    story.append(Paragraph(
    ...
    # ================= 10. FUTURE WORK =================
    story.append(Paragraph("10. Future Work & Deployment", h1_style))

    story.append(Paragraph("Q1: What would improve if given another month?", q_style))
    story.append(Paragraph(
        "A: Integrate cryptographic secure aggregation schemes (SecAgg), implement multi-round federated training loops, "
        "and benchmark the model on physical edge hardware (Raspberry Pi/Jetson).",
        a_style
    ))

    story.append(Paragraph("Q2: How would you deploy this?", q_style))
    story.append(Paragraph(
    ...
    story.append(Paragraph("Q3: How would this work in production?", q_style))
    story.append(Paragraph(
        "A: Local Edge Gateways run real-time packet sniffer modules, scale feature vectors in standard standardizers, "
        "generate sequence arrays, run local autoencoder reconstruction inference, flag anomalies, and stream weights "
        "periodically.",
        a_style
    ))

    story.append(Paragraph("Q4: How would you secure communication?", q_style))
    story.append(Paragraph(
        "A: Implement TLS encryption for weight transfers, establish mutual authentication (mTLS) between Edges and Gateways, "
        "and utilize secure multi-party aggregation.",
        a_style
    ))

    story.append(Paragraph("Q5: How would you scale to thousands of IoT devices?", q_style))
    story.append(Paragraph(
        "A: By introducing a hierarchical cluster mesh of regional gateway clusters, scaling the aggregation nodes via "
        "Kubernetes, and grouping client uploads into random mini-batches of updates during federated training.",
        a_style
    ))

    story.append(Paragraph("Q6: Would you use Kafka?", q_style))
    story.append(Paragraph(
        "A: Yes, Apache Kafka acts as a high-throughput, fault-tolerant message broker to ingest packet flows and distribute "
        "inference tasks to regional NIDS nodes.",
        a_style
    ))

    story.append(Paragraph("Q7: Would you use Docker?", q_style))
    story.append(Paragraph(
        "A: Yes. Containerizing the Node services ensures consistent environment setups, dependency isolation, and rapid "
        "deployments on edge gateways.",
        a_style
    ))

    story.append(Paragraph("Q8: Would you use Kubernetes?", q_style))
    story.append(Paragraph(
        "A: Yes, Kubernetes manages and scales aggregator nodes, handles node failures automatically, and balances workloads "
        "in dynamic networks.",
        a_style
    ))
    ...
    story.append(PageBreak())
    story.append(Paragraph("11. Resume Verification", h1_style))

    story.append(Paragraph("Q1: Why did you update the project recently?", q_style))
    story.append(Paragraph(
        "A: Because the early design had three major weaknesses: target feature leakage, sequential test split bias (Zero Support trap), "
        "and a Gateway weight-overwriting bug. I refactored the pipeline to make it mathematically valid and recruiter-ready.",
        a_style
    ))
    ...
    story.append(Paragraph("Q3: Which version should we consider?", q_style))
    story.append(Paragraph(
        "A: The current unsupervised, leak-free version. It is technically robust and represents clean ML engineering.",
        a_style
    ))
    ...
    story.append(Paragraph("Q5: Did someone help you build this?", q_style))
    story.append(Paragraph(
        "A: I designed and wrote the code myself, utilizing PyTorch, Scikit-Learn, and SHAP documentation to solve "
        "specific architecture and preprocessing problems.",
        a_style
    ))

    story.append(Paragraph("Q6: Which parts did you personally implement?", q_style))
    story.append(Paragraph(
        "A: The entire pipeline: dataset loaders, model architectures, weight quantization, FedAvg loops, Gateway buffering, "
        "and SHAP explainability visualizations.",
        a_style
    ))

    # ================= 12. CURVEBALL QUESTIONS =================
    story.append(Paragraph("12. Curveball Questions", h1_style))
    ...
    story.append(Paragraph("Q2: What assumptions does your model make?", q_style))
    story.append(Paragraph(
        "A: It assumes that normal network behavior is relatively consistent over time, and that attacks will deviate "
        "significantly from this benign pattern.",
        a_style
    ))

    story.append(Paragraph("Q3: What are its limitations?", q_style))
    story.append(Paragraph(
        "A: If an attack occurs extremely slowly and mimic normal traffic patterns exactly, the reconstruction error "
        "will not spike, and the attack may go undetected.",
        a_style
    ))

    story.append(Paragraph("Q4: When would your model fail?", q_style))
    story.append(Paragraph(
        "A: It would fail if the local training splits at Edge nodes were contaminated with attack packets, causing the model "
        "to learn to reconstruct attacks with low error.",
        a_style
    ))

    story.append(Paragraph("Q5: If attack patterns evolve over time, what happens?", q_style))
    story.append(Paragraph(
        "A: Since the model detects anomalies based on deviation from normal traffic, evolved attack patterns will still be "
        "flagged as anomalies. However, normal traffic drift would require periodic model retraining.",
        a_style
    ))

    story.append(Paragraph("Q6: Can your model detect zero-day attacks?", q_style))
    story.append(Paragraph(
        "A: Yes. Because it is unsupervised and only learns to reconstruct normal traffic, any zero-day attack will deviate "
        "from the normal baseline, yield high reconstruction error, and be flagged.",
        a_style
    ))

    story.append(Paragraph("Q7: Why didn't you use supervised learning?", q_style))
    story.append(Paragraph(
        "A: Supervised learning requires constant labeling of attacks and fails to detect novel (zero-day) attacks. Unsupervised "
        "learning is more robust for network environments.",
        a_style
    ))

    story.append(Paragraph("Q8: Why didn't you compare against a centralized baseline?", q_style))
    story.append(Paragraph(
        "A: In our IoT network scenario, centralizing raw traffic logs is prohibited due to privacy policies. A centralized baseline "
        "would violate the core project constraint.",
        a_style
    ))

    story.append(Paragraph("Q9: What if communication latency is very high?", q_style))
    story.append(Paragraph(
        "A: High latency slows down the global model updates. However, local inference at Edge nodes continues uninterrupted "
        "using the existing local model weights.",
        a_style
    ))

    # ================= 13. CODING QUESTIONS =================
    story.append(PageBreak())
    story.append(Paragraph("13. Coding Questions", h1_style))
    ...
    story.append(Paragraph("Q2: Write PyTorch code to compute reconstruction error.", q_style))
    code_snippet = (
        "import torch\n"
        "import torch.nn as nn\n"
        "\n"
        "def compute_mse_reconstruction(model, x_tensor):\n"
        "    model.eval()\n"
        "    criterion = nn.MSELoss(reduction='none')\n"
        "    with torch.no_grad():\n"
        "        reconstructed = model(x_tensor)\n"
        "        # Mean over sequence length and feature dimension\n"
        "        mse = criterion(reconstructed, x_tensor).mean(dim=[1, 2])\n"
    ...
    story.append(Paragraph("Q4: Explain your preprocessing pipeline.", q_style))
    story.append(Paragraph(
        "A: 1. Impute missing values with mean (numeric) and mode (categorical). 2. Handle infinite values. 3. Target isolate "
        "(Label) and filter out leakage (Attack_categories). 4. Scale continuous features using StandardScaler. 5. Apply "
        "VarianceThreshold and SelectKBest to keep the top 25 features.",
        a_style
    ))

    story.append(Paragraph("Q5: Explain StratifiedKFold.", q_style))
    story.append(Paragraph(
        "A: A cross-validation splitter that folds the dataset while ensuring that the percentage of samples for each class "
        "label is identical across all folds.",
        a_style
    ))

    story.append(Paragraph("Q6: Explain your train-test split.", q_style))
    story.append(Paragraph(
        "A: We split the dataset into Train Part A (70%) and Test Part B (30%). Train Part A is split into A.1 and A.2 for "
        "Edge nodes. Test Part B is partitioned into three equal, stratified test sets (B.1, B.2, B.3) for evaluations.",
        a_style
    ))

    # ================= 14. HR + TECHNICAL MIX =================
    story.append(Paragraph("14. HR + Technical Mix", h1_style))

    story.append(Paragraph("Q1: Which part are you most proud of?", q_style))
    story.append(Paragraph(
        "A: Identifying and fixing the target leakage bug. It required deep-diving into the preprocessing encoding logic "
        "and taught me that system robustness depends on data integrity, not just model hyper-parameters.",
        a_style
    ))

    story.append(Paragraph("Q2: What would you redesign today?", q_style))
    story.append(Paragraph(
        "A: I would redesign the Gateway upload logic to run asynchronously using threading or async/await, allowing clients "
        "with different hardware configurations to upload their updates as soon as they finish.",
        a_style
    ))

    story.append(Paragraph("Q3: If another engineer joins, what would you improve first?", q_style))
    story.append(Paragraph(
        "A: I would improve documentation and implement automated unit tests verifying the pipeline (e.g. sanity checking "
        "that weights aggregate correctly and test splits are stratified).",
        a_style
    ))

    story.append(Paragraph("Q4: What did this project teach you?", q_style))
    story.append(Paragraph(
        "A: It taught me the complexities of distributed learning, target leakage hazards, and the necessity of keeping evaluations "
        "realistic using stratified splits.",
        a_style
    ))

    story.append(Paragraph("Q5: If I give you six months, how would you turn this into a real product?", q_style))
    story.append(Paragraph(
        "A: I would port the preprocessors and PyTorch models to ONNX Runtime for edge deployment, integrate TLS/mTLS secure weight "
        "relay communication protocols, deploy nodes using Docker/Kubernetes, route logs via Apache Kafka, and build an alert "
    ...
    story.append(PageBreak())
    story.append(Paragraph("15. Rapid Fire Explanations", h1_style))
    ...
    story.append(Paragraph("16. Common Mistakes to Avoid", h1_style))
